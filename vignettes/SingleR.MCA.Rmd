...
mca.singler$labels = paste(mca.singler$Main.Types,mca.singler$Types,mca.singler$Orig.Ident,sep='/')
...
p = ggplot(mca.singler)+geom_point_rast(aes(x=FItSNE_1,y=FItSNE_2,color=Main.Types),size=0.1,alpha=0.5)+
  guides(color=guide_legend(override.aes = list(size=2,alpha=1)))+theme_classic()
p
```

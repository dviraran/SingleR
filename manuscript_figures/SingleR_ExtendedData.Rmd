...
**Figure S1. 8,366 single-cells colored by batches.** This t-SNE plot accompanies Figure 1c. The colors are coded by batch. All batches intersect, suggesting that our data does not suffer from a strong batch effect. 
...
n = table(labels)
labels[labels %in% names(n)[n<15]]='*Others*'
...
**Figure S2. SingleR detailed annotations.** This heatmap accompanies Figure 1b. Here we present all SingleR annotations (not just main types). Interestingly, SingleR annoated the cells in the middle of macrophages cluster as alveolar macrophages extracted from a PPARG- mouse, opposed to the top cells which were annotated is the WT alveolar macrophages. To visualize the data, annotations with less than 20 cells were marked as *Others* and colored in black.
...
load(file.path(path.data,'decon.rds'))
...
include_graphics(file.path(path.data,'Supp_Fig_7.pdf'))
```

#### b
\vspace*{-4mm}
```{r message=FALSE,warning=FALSE,echo=F,fig.height=2.5,fig.width=4,fig.align='center',eval=T}
df = read.table(file.path(path.data,'supp_fig7_data.txt'), header=TRUE, sep="\t", row.names=NULL, as.is=TRUE)
df = melt(df)
colnames(df) = c('Group','value')
df$Group = mapvalues(factor(df$Group),from=c('CTRL','BLEO','BLEO.DTA'),to=c('Control','Bleomycin','Bleomycin+DTA'))
ggplot(df,aes(x=Group,y=value,color=Group,fill=Group))+
  ...
  ylim(0,140)+
  stat_compare_means(label.x.npc=0.4,label='p.format',comparisons=list(c('Control','Bleomycin'),c('Bleomycin','Bleomycin+DTA')))+ ylab('# of positive Mertk or SiglecF cells')
```
...
```{r message=FALSE,warning=FALSE,echo=F}
tbl.s1 <- read.csv(file.path(path.data,'table s1.csv'),header=TRUE, row.names=NULL, as.is=TRUE)
...
```{r message=FALSE,warning=FALSE,echo=F}
tbl.s3 <- read.csv(file.path(path.data,'table_s3.csv'),header=TRUE, row.names=NULL, as.is=TRUE)
